...
3.  **[code/03_Framework1_Analysis.R](file:///home/j/AgenticProjects/DefaunationSynthesis/code/03_Framework1_Analysis.R):**  
    Performs covariate model selection for GEDI understory openness (UOI) using **weighted Beta Regressions** across **68 candidate models** (incorporating environmental covariates, basin interactions, and standing biomass sub-components such as megafauna >100 kg [`B_H_gt100`] and >1000 kg [`B_H_gt1000`]). The selected best-fitting model is **`M26: Biomass + Basin + Elev + Megafauna`** (`uoi ~ B_H_index + basin + elevation + B_H_gt100`), showing that large physical engineers exert a distinct physical structuring effect on canopy openness. Generates the publication-quality **Figure 3 (figure3.png)** with a continuous, vibrant model selection bar plot.
